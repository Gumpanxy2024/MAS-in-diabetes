...
## 2026-04-16 前端架构重构 & 对话式 AI 助手后端接入

### 背景
另一个窗口完成了 `FRONTEND_PLAN.md` 的根本性重组：患者端从传统表单/仪表盘范式切换到**对话式 AI 健康助手**作为默认首页。

### 前端架构变化摘要

| 之前 | 现在 |
|------|------|
| P-01 健康概览（首页） | **P-01 AI 健康助手**（首页，对话式） |
| P-02 双面板录入页 | 体征录入内嵌在对话流中 |
| P-03 评估结果（跳转新页面） | 评估结果内嵌在对话流中 |
| P-07 AI 交互历史（独立页） | 即 P-01 对话历史 |
| RAG 问答无独立入口 | **P-01 核心功能**：随时提问 |

Tab 导航：AI助手 / 概览 / 记录 / 用药 / 随访

### 本次后端变更

1. **新增 `POST /patient/api/chat/` 接口**（`patients/views.py`）
   - 接收患者自由提问 → RAG 知识库检索（audience=patient, top_k=3）→ LLM 生成回答
   - 问答记录持久化至 AgentLog（log_type=rag_chat）
   - 返回 JSON：`{ success, answer, sources }`

2. **URL 路由注册**（`patients/urls.py`）
   - 新增 `path("api/chat/", views.chat_api, name="api_chat")`

### 论文章节同步更新

**ch03 需求分析：**
- 3.1.1「健康数据采集需求」：重写为对话式交互范式，强调"打开即对话、一句话完成全部操作"
- 3.1.6 改名「知识辅助与AI对话交互需求」：新增"对话式AI健康助手"子需求，包含健康问答、体征录入、语音播报三项功能
- 3.3.3 新增用例「AI健康问答」：描述患者在对话中提问→RAG检索→LLM回答的完整流程
- 3.3.1 用例「语音录入健康数据」→ 改名「通过AI助手录入健康数据」：体现对话式预览卡片确认流程
- 3.3.4 用例「查看健康反馈建议」：更新为对话流内嵌展示

**ch04 系统设计：**
- 4.3.3「健康数据管理模块」：全面重写，包含对话式体征录入、评估结果内嵌展示、对话式RAG健康问答三部分
- 4.3.7 RAG服务接口表：从2个接口扩展为3个，新增 `chat_api()` 行
- 4.2.4 PatientAgent：健康反馈改为"内嵌在对话流中"
- AgentLog 日志类型：从7种扩展为8种，新增 `rag_chat`
